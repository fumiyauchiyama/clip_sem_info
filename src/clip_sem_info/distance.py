...
def angular_disrance(x: TT["hidden_dim"], y: TT["hidden_dim"]) -> TT["num_sample"]: # noqa: F821
    return torch.acos(torch.clamp(F.cosine_similarity(x, y, dim=-1), min=-1, max=1)) # type: ignore


def euclidean_disrance(x: TT["hidden_dim"], y: TT["hidden_dim"]): # noqa: F821
